...
function ProvinciasTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { provincias, error, isLoading, isValidating, refresh } = useProvincias(); // EDITABLE
    ...
    useEffect(() => {
        const savedState = sessionStorage.getItem('prov-table-state');
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.autoTable({
                    ...
                    body: provincias.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                });

                doc.save('Provincias.pdf'); // EDITABLE
            });
        });
    }; // Función para exportar a PDF: ESPECIFICO
    const exportExcel = () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Provincias'); // EDITABLE
        ...
        provincias.forEach((prov) => { //EDITABLE
            worksheet.addRow([
                prov.id,
                prov.nombre,
            ]);
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Provincias'); // EDITABLE
        });
    ...
    return (
        <div className="table">
            ...
            <DataTable
                ...
                dataKey="id" // EDITABLE
                filters={filters}
                globalFilterFields={['id', 'nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron provincias" // EDITABLE
                ...
                stateKey="prov-table-state" // EDITABLE
            >
...
export default ProvinciasTable;