...
function EspeciesTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { especies, error, isLoading, isValidating, refresh } = useEspecies(); // EDITABLE
    ...
    useEffect(() => {
        const savedState = sessionStorage.getItem('espe-table-state');
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.autoTable({
                    ...
                    body: especies.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                });

                doc.save('Especies.pdf'); // EDITABLE
            });
        });
    }; // Función para exportar a PDF: ESPECIFICO
    const exportExcel = () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Especies'); // EDITABLE
        ...
        especies.forEach((espe) => { //EDITABLE
            worksheet.addRow([
                espe.id,
                espe.especie,
            ]);
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Especies'); // EDITABLE
        });
    ...
    return (
        <div className="table">
            ...
            <DataTable
                ...
                dataKey="id" // EDITABLE
                filters={filters}
                globalFilterFields={['id', 'especie']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron especies" // EDITABLE
                ...
                stateKey="espe-table-state" // EDITABLE
            >
