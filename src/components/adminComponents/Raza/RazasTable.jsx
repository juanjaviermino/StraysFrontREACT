...
function RazasTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { razas, error, isLoading, isValidating, refresh } = useRazas(); // EDITABLE
    ...
    useEffect(() => {
        const savedState = sessionStorage.getItem('raza-table-state');
    ...
    const initFilters = () => {
        setFilters({
            ...
            raza: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.autoTable({
                    ...
                    body: razas.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                });

                doc.save('Razas.pdf'); // EDITABLE
            });
        });
    }; // Función para exportar a PDF: ESPECIFICO
    const exportExcel = () => {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Razas'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Razas'); // EDITABLE
        });
    ...
    const columnsData = [
        ...
        {
            nombrevar: "raza",
            header: "Raza de mascota",
            filterPlaceholder: "Buscar por raza",
        },
        {
            nombrevar: "especie",
            header: "Especie de la mascota",
            filterPlaceholder: "Buscar por especie",
        }
    ...
    return (
        <div className="table">
            ...
            <DataTable
                ...
                dataKey="id" // EDITABLE
                filters={filters}
                globalFilterFields={['id', 'raza', 'especie']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron razas" // EDITABLE
                ...
                stateKey="raza-table-state" // EDITABLE
            >
